...
use Config\Services;
use App\Models\Modeltempatkerja;
use App\Models\Modelmaster;
class Tempatkerja extends BaseController{
    public function index()
    {
        ...
        // }
        $data = [
            'title' => 'Data Tempat Kerja',
        ];
        return view('auth/tempatkerja/index', $data);
    }
    ...
    public function getdata()
    {
        if ($this->request->isAJAX()) {
            $data = [
                'title' => 'List Data Tempat Kerja',
                'list' => $this->tempatkerja->list()

            ];
            $msg = [
                'data' => view('auth/tempatkerja/list', $data)
            ];
            echo json_encode($msg);
        }
    }

    public function getdatatempatkerja()
    {
        $request = Services::request();
        $datamodel = $this->tempatkerja;
        if ($request->getMethod()) {
            ...
            foreach ($lists as $list) {
                ...
                $row = [];
                $edit = "<button type=\"button\" class=\"btn btn-primary btn-sm\" onclick=\"edit('" . $list->tempatkerja_id . "')\">
                <i class=\"fa fa-edit\"></i>
            </button>";
                $hapus = "<button type=\"button\" class=\"btn btn-danger btn-sm\" onclick=\"hapus('" . $list->tempatkerja_id . "')\">
                <i class=\"fa fa-trash\"></i>
            </button>";

                // $row[] = "<input type=\"checkbox\" name=\"tempatkerja_id[]\" class=\"centangtempatkerjaid\" value=\"$list->tempatkerja_id\">";
                $row[] = $no;
    ...
    public function formtambah()
    {
        if ($this->request->isAJAX()) {
            $data = [
                'title' => 'Tambah Data Tempat kerja',
            ];
            $msg = [
                'data' => view('auth/tempatkerja/tambah', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function simpan()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $this->tempatkerja->insert($simpandata);
                $msg = [
    ...
    {
        if ($this->request->isAJAX()) {
            $tempatkerja_id = $this->request->getVar('tempatkerja_id');
            $list =  $this->tempatkerja->find($tempatkerja_id);
            $data = [
                'title'         => 'Edit Data Tempat kerja',
                'tempatkerja_id'      => $list['tempatkerja_id'],
            ...
            $msg = [
                'sukses' => view('auth/tempatkerja/edit', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function update()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $tempatkerja_id = $this->request->getVar('tempatkerja_id');
                $this->tempatkerja->update($tempatkerja_id, $updatedata);
                $msg = [
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $tempatkerja_id = $this->request->getVar('tempatkerja_id');
            //check
            $cekdata = $this->tempatkerja->find($tempatkerja_id);

            $this->tempatkerja->delete($tempatkerja_id);
            $msg = [
                'sukses' => 'Data tempat tempatkerja Berhasil Dihapus'
            ];
    ...
    public function hapusall()
    {
        if ($this->request->isAJAX()) {
            $tempatkerja_id = $this->request->getVar('tempatkerja_id');
            $jmldata = count($tempatkerja_id);
            for ($i = 0; $i < $jmldata; $i++) {
                //check
                $cekdata = $this->tempatkerja->find($tempatkerja_id[$i]);
