...
class Pendidikan extends BaseController{
    public function index()
    {
        ...
        return view('auth/pendidikan/index', $data);
    }
    ...
    public function getdata()
    {
        if ($this->request->isAJAX()) {
            $data = [
                ...
                'list' => $this->pendidikan->list()

            ];
            $msg = [
                'data' => view('auth/pendidikan/list', $data)
            ];
    ...
    public function getdatapendidikan()
    {
        ...
        if ($request->getMethod()) {
            ...
            foreach ($lists as $list) {
                ...
                $row = [];
                $edit = "<button type=\"button\" class=\"btn btn-primary btn-sm\" onclick=\"edit('" . $list->pendidikan_id . "')\">
                <i class=\"fa fa-edit\"></i>
            </button>";
                $hapus = "<button type=\"button\" class=\"btn btn-danger btn-sm\" onclick=\"hapus('" . $list->pendidikan_id . "')\">
                <i class=\"fa fa-trash\"></i>
            </button>";

                // $row[] = "<input type=\"checkbox\" name=\"pendidikan_id[]\" class=\"centangpendidikanid\" value=\"$list->pendidikan_id\">";
                $row[] = $no;
    ...
    public function formtambah()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('auth/pendidikan/tambah', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function formedit()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('auth/pendidikan/edit', $data)
            ];
            echo json_encode($msg);
        }
    }
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $pendidikan_id = $this->request->getVar('pendidikan_id');
            //check
            $cekdata = $this->pendidikan->find($pendidikan_id);

            $this->pendidikan->delete($pendidikan_id);
            $msg = [
                'sukses' => 'Data tempat pendidikan Berhasil Dihapus'
            ];
    ...
    public function hapusall()
    {
        if ($this->request->isAJAX()) {
            $pendidikan_id = $this->request->getVar('pendidikan_id');
            $jmldata = count($pendidikan_id);
            for ($i = 0; $i < $jmldata; $i++) {
                //check
                $cekdata = $this->pendidikan->find($pendidikan_id[$i]);
